...
entire_linear_preds <- all_lin_preds_new

entire_linear_preds <- entire_linear_preds[match(target, entire_linear_preds$fullVisitorId), ]
  # source: https://stackoverflow.com/questions/11977102/order-data-frame-rows-according-to-vector-with-specific-order
names(entire_linear_preds) <- c('fullVisitorId', 'PredictedLogRevenue')
...
entire_linear_preds$fullVisitorId <- as.character(entire_linear_preds$fullVisitorId)
length(entire_linear_preds$fullVisitorId)

# Write the results to the .csv
write.csv(entire_linear_preds, row.names=F, file = "New_Linear_Predictions.csv")


##### Linear Spline #####
# Use the same cross-validation method 
# Take our best linear model and 
...
target <- submission$fullVisitorId
spline_preds <- all_spline_preds

spline_preds <- spline_preds[match(target, spline_preds$fullVisitorId), ]
names(spline_preds) <- c('fullVisitorId', 'PredictedLogRevenue')

spline_preds$fullVisitorId <- as.character(spline_preds$fullVisitorId)

# Write to csv
write.csv(spline_preds, row.names=F, file='Spline_predictions.csv')
